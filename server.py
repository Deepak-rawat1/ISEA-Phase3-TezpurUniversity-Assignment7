...
LOCKOUT_DURATION = config["security"]["lockout_duration_seconds"]
SESSION_TIMEOUT = config["security"]["session_timeout_seconds"]
MAX_MSG_LEN = config["security"]["max_message_length"]
MIN_USER_LEN = config["security"]["min_username_length"]
MAX_USER_LEN = config["security"]["max_username_length"]
...
def ensure_file_headers():
    ...
    if not os.path.exists(PERFORMANCE_FILE) or os.path.getsize(PERFORMANCE_FILE) == 0:
        with open(PERFORMANCE_FILE, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["timestamp", "clients", "broadcast_messages", "private_messages", "avg_delay_ms", "throughput_msgs_per_sec"])
...
if not os.path.exists(CREDENTIALS_FILE):
    default_users = {
        ...
        "admin": hashlib.sha256("AdminPass789".encode()).hexdigest()
    }
    for i in range(1, 15):
        default_users[f"user{i}"] = hashlib.sha256(f"Password{i}".encode()).hexdigest()
...
def log_server(event, username, ip):
    with open(SERVER_LOG_FILE, "a") as f:
        f.write(f"[{now()}] EVENT: {event} | USERNAME: {username} | IP: {ip}\n")
...
def record_performance_telemetry():
    global session_start_time, session_message_count, session_broadcast_count, session_private_count, session_total_delay
    while running:
        time.sleep(10)
        with lock:
            current_count = len(active_clients)
            if current_count == 0 and session_message_count == 0:
                continue
            duration = time.time() - (session_start_time or time.time())
            throughput = session_message_count / duration if duration > 0 else 0.0
            avg_delay = (session_total_delay / session_message_count * 1000) if session_message_count > 0 else 0.0
            
            with open(PERFORMANCE_FILE, "a", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([now(), current_count, session_broadcast_count, session_private_count, round(avg_delay, 2), round(throughput, 2)])
...
def validate_inputs(username, password):
    if not username.isalnum() or len(username) < MIN_USER_LEN or len(username) > MAX_USER_LEN:
        return False, f"ERROR: Username must be alphanumeric and between {MIN_USER_LEN}-{MAX_USER_LEN} chars."
    if not password:
    ...
    while running:
        try:
            ...
            if len(text) > MAX_MSG_LEN:
                safe_send(client_sock, f"ERROR: Message payload exceeds limit of {MAX_MSG_LEN} characters.")
...
def main():
    ...
    try:
        ...
        server.listen(20) # Backlog optimized for scale
        print(f"[*] Secure Optimised Server running on port {PORT}")
    except Exception as e:
        print(f"[-] Binding exception: {e}")
        return

    threading.Thread(target=monitor_idle_sessions, daemon=True).start()
    threading.Thread(target=record_performance_telemetry, daemon=True).start()
    ...
    try:
        ...
        running = False
        with lock:
            for sock in list(active_clients.keys()):
                safe_send(sock, "SERVER_SHUTDOWN: The server is closing down gracefully.")
                try: sock.close()
                except: pass
        server.close()
