...
class ChatClientGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Multi-Client Chat Client")
    ...
    def _build_styles(self):
        ...
        self.option_add("*TCombobox*Listbox.background", "#252526")
        self.option_add("*TCombobox*Listbox.foreground", "#ffffff")
        self.option_add("*TCombobox*Listbox.selectBackground", "#2d7dd2")
    ...
    def _build_login_frame(self):
        ...
        header.pack(fill="x", pady=(10, 22))

        ttk.Label(header, text="Multi-Client Chat Client", style="Title.TLabel").pack(anchor="w")
        ttk.Label(
            header,
            text="Connect to the TCP chat server and send broadcast or private messages.",
        ...
        ttk.Label(card, text="Username", style="CardTitle.TLabel").grid(row=0, column=0, sticky="w", pady=(0, 8))
        ttk.Entry(card, textvariable=self.username, font=("Segoe UI", 11), width=36).grid(row=1, column=0, sticky="ew", pady=(0, 16))

        ttk.Label(card, text="Server IP", style="CardTitle.TLabel").grid(row=2, column=0, sticky="w", pady=(0, 8))
        ttk.Entry(card, textvariable=self.server_ip, font=("Segoe UI", 11), width=36).grid(row=3, column=0, sticky="ew", pady=(0, 16))

        ttk.Label(card, text="Port", style="CardTitle.TLabel").grid(row=4, column=0, sticky="w", pady=(0, 8))
        ttk.Entry(card, textvariable=self.server_port, font=("Segoe UI", 11), width=36).grid(row=5, column=0, sticky="ew", pady=(0, 20))

        btns = ttk.Frame(card, style="Card.TFrame")
        btns.grid(row=6, column=0, sticky="e")
        ttk.Button(btns, text="Connect", command=self.connect_to_server).pack(side="right")
    ...
    def _build_chat_frame(self):
        ...
        ttk.Label(left_top, text="Multi-Client Chat Client", style="Title.TLabel").pack(anchor="w")
        ttk.Label(left_top, textvariable=self.status_text, style="Status.TLabel").pack(anchor="w", pady=(4, 0))

        ttk.Button(topbar, text="Disconnect", command=self.disconnect).pack(side="right")
        ...
        ttk.Label(chat_card, text="Chat Messages", style="CardTitle.TLabel").pack(anchor="w", pady=(0, 8))
        ...
        self.recipient_combo.bind("<<ComboboxSelected>>", self._recipient_changed)
    ...
    def update_users(self, users):
        users = sorted(set(u for u in users if u and u != self.username.get()), key=str.lower)
    ...
    def _recipient_changed(self, _event=None):
        self.message_entry.focus_set()

    def _on_user_double_click(self, _event=None):
    ...
    def receive_messages(self):
        while not self.stop_event.is_set():
            try:
                ...
                self.after(0, self.process_incoming_message, text)
            except Exception:
                break
